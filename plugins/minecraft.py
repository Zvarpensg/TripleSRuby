...
class MCQuery:
    ...
    def write_packet(self, type, payload):
        o = '\xFE\xFD' + struct.pack('>B', type).decode('UTF-8') + struct.pack('>l', self.id).decode('UTF-8') + payload
        self.socket.sendto(o.encode('UTF-8'), self.addr)
    ...
    def extra_stat(self, *values):
        #Data packets tell the server which values you want
 
        dString = json.dumps(values)
 
        self.write_packet(8, self.challenge + dString)
        try:
            type, id, buff = self.read_packet()
        except:
            self.handshake()
            return self.extra_stat()
...
print(q.extra_stat("time", {"key":"players", "args":"latestdeath"}, "uptime", "dimensions", {"key":"time", "args":1}, {"key":"time", "args":-1}, "deaths", {"key":"deaths", "args":"Sky_Som"}))
